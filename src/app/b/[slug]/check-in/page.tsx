...
export default function BranchCheckInPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/b/${params.slug}/check-in/success`)
    } catch (error) {
      console.error(error)
  ...
  if (!branch) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-4 text-center">
        ...
        <p className="mt-2 text-gray-500">The branch "{params.slug}" does not exist.</p>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="bg-[#1B4332] px-4 py-6 text-center">
        ...
        <p className="mt-1 text-sm text-green-200">{branch.name} — Welcome! Please check in below.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mx-auto max-w-md space-y-5 px-4 py-6">
        ...
        <div className="space-y-1.5">
          ...
          <Input id="city" value={city} onChange={(e) => setCity(e.target.value)} placeholder="Mabalacat City" required className="h-10" />
        </div>
      ...
      <div className="fixed bottom-4 right-4">
        <a href="/login" className="rounded-md bg-gray-100 px-3 py-1.5 text-xs text-gray-400 hover:bg-gray-200 hover:text-gray-600 transition-colors">
          Staff Login →
        </a>
      </div>
    </div>
